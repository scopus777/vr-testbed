...
inline FragmentCommonData FragmentSetup (inout float4 i_tex, float3 i_eyeVec, half3 i_viewDirForParallax, float4 tangentToWorld[3], float3 i_posWorld)
{
    ...
    // BEGIN CHANGE
    // check whether voxel lies in the boundary cube
    // Source: https://stackoverflow.com/questions/21037241/how-to-determine-a-point-is-inside-or-outside-a-cube
    // half alpha = Alpha(i_tex.xy);
    half alpha;
    float3 dir = i_posWorld - _BoundariesCenter;
    float px = abs(dot(dir,_DirVectorX));
    float py = abs(dot(dir,_DirVectorY));
    float pz = abs(dot(dir,_DirVectorZ));
    if (2* px <= _BoundariesSize.x && 2*py <= _BoundariesSize.y && 2*pz <= _BoundariesSize.z) 
